...
export class NavbarComponent {
  ...
  constructor(){
    ...
    this.router.events.pipe(filter(e => e instanceof NavigationEnd)).subscribe((e: any) => {
      ...
      if (url.startsWith('/superadmin')) {
        if (!this.auth.hasRole(ROLES.SUPERADMIN)) this.auth.switchActiveRole(ROLES.SUPERADMIN);
      } else if (url.startsWith('/admin')) {
        if (!this.auth.hasRole(ROLES.ADMIN)) this.auth.switchActiveRole(ROLES.ADMIN);
      } else if (url.startsWith('/seguridad')) {
        if (!this.auth.hasRole(ROLES.SEGURIDAD)) this.auth.switchActiveRole(ROLES.SEGURIDAD);
      } else if (url.startsWith('/usuario')) {
        if (!this.auth.hasRole(ROLES.USUARIO)) this.auth.switchActiveRole(ROLES.USUARIO);
      }
    });
  ...
  changeModule(mod: string) {
    if (mod === 'login') {
      this.router.navigate(['/login']);
    } else if (mod === 'superadmin') {
      this.auth.switchActiveRole(ROLES.SUPERADMIN);
      this.router.navigate(['/superadmin/dashboard']);
    } else if (mod === 'admin') {
      this.auth.switchActiveRole(ROLES.ADMIN);
      this.router.navigate(['/admin']);
    } else if (mod === 'seguridad') {
      this.auth.switchActiveRole(ROLES.SEGURIDAD);
      this.router.navigate(['/seguridad']);
    } else if (mod === 'usuario') {
      this.auth.switchActiveRole(ROLES.USUARIO);
