...
export class DarkModeService {
  ...
  constructor() {
    ...
    // Compute current role key and initialize dark mode (defaulting to light mode false)
    this.roleKey = this.computeRoleKey();
    const stored = this.map[this.roleKey];
    if (this.roleKey === 'SUPERADMIN') {
      // Forzar siempre modo claro para SUPERADMIN
      this.darkMode.set(false);
    } else {
      if (typeof stored === 'boolean') this.darkMode.set(stored);
      else this.darkMode.set(false); // Por defecto MODO CLARO en el prototipo
    }
