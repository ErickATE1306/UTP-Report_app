...
export class ReportesSensibles {
  ...
  private readonly SECRET_KEY: string = '123456';
  ...
  // Validación rápida contra la clave de paso
  async submitAuth() {
    this.authError = null;

    if (!this.passwordInput || !this.passwordInput.trim()) {
      this.authError = 'Ingrese la clave de acceso';
      return;
    }

    this.loadingAuth = true;

    // Simulación de respuesta rápida para UX
    setTimeout(() => {
      if (this.passwordInput === this.SECRET_KEY) {
        this.accesoConcedido = true;
        this.showAuthModal = false;
        this.passwordInput = ''; // Se limpia la clave ingresada por seguridad
        this.onAccessGranted();
      } else {
