...
export class AuthService {
  ...
  loadFromStorage() {
    ...
    if (!token) {
      this.isAuthenticated.set(true);
      const defaultRoles = [ROLES.USUARIO];
      const stored = localStorage.getItem('auth_roles');
      if (stored) {
        try { this.rolesSignal.set(JSON.parse(stored)); } catch (_) { this.rolesSignal.set(defaultRoles); }
      } else {
        this.rolesSignal.set(defaultRoles);
        localStorage.setItem('auth_roles', JSON.stringify(defaultRoles));
      }
      return;
    }
    this.isAuthenticated.set(true);
  ...
  // Permite al selector de módulos en la barra superior cambiar el rol activo en el prototipo estático
  switchActiveRole(role: string) {
    if (role === 'ALL') {
      const roles = [ROLES.SUPERADMIN, ROLES.ADMIN, ROLES.USUARIO, ROLES.SEGURIDAD];
      this.rolesSignal.set(roles);
      this.setAdminActingAsUser(false);
      localStorage.setItem('auth_roles', JSON.stringify(roles));
    } else if (role === ROLES.SUPERADMIN) {
      this.rolesSignal.set([ROLES.SUPERADMIN]);
      this.setAdminActingAsUser(false);
      localStorage.setItem('auth_roles', JSON.stringify([ROLES.SUPERADMIN]));
    } else if (role === ROLES.ADMIN) {
      this.rolesSignal.set([ROLES.ADMIN]);
      this.setAdminActingAsUser(false);
      localStorage.setItem('auth_roles', JSON.stringify([ROLES.ADMIN]));
    } else if (role === ROLES.SEGURIDAD) {
      this.rolesSignal.set([ROLES.SEGURIDAD]);
      this.setAdminActingAsUser(false);
      localStorage.setItem('auth_roles', JSON.stringify([ROLES.SEGURIDAD]));
    } else if (role === ROLES.USUARIO) {
      this.rolesSignal.set([ROLES.USUARIO]);
      this.setAdminActingAsUser(false);
      localStorage.setItem('auth_roles', JSON.stringify([ROLES.USUARIO]));
    }
  }
